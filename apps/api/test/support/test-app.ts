...
import { InventoryClient } from '../../src/application/ports/inventory-client.port';
...
/** Optional provider overrides accepted by {@link createTestApp}. */
export interface CreateTestAppOptions {
  /**
   * Replaces the real `InventoryClient` (`HttpInventoryClient`, which calls
   * `apps/inventory` over the network) with an in-memory test double. Needed
   * by any spec that exercises `/admin/inventory/:productId`, since
   * `apps/inventory` has no host port published and is unreachable from the
   * host-side Jest process.
   */
  fakeInventoryClient?: FakeInventoryClient;
}
...
export async function createTestApp(
  ...
  if (options?.fakeInventoryClient) {
    builder.overrideProvider(InventoryClient).useValue(options.fakeInventoryClient);
  }
