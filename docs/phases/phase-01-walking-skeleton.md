...
- [ ] Root `docker-compose.yml`: postgres (named volume, healthcheck, host port 5432), nginx (host port 8080), five apps with `/health` healthchecks and `depends_on`; only nginx and postgres publish host ports (owner: main)
