...
- [ ] Add `redis` service to docker-compose (`redis:alpine`, healthcheck, host port 6379); give `api` the `DATABASE_URL`, `REDIS_URL`, and JWT env vars with compose defaults and `depends_on` healthy postgres + redis; update `.env.example` (owner: main)
- [ ] Add shared API contract types to `packages/types`: `PaginationMeta`, `ListResponse<T>`, `SingleResponse<T>`, `ErrorResponse` (owner: main)
